...
/**
 * Parses CREATE CONSTRAINT TRIGGER statements.
 *
 * @author nromero
 */
public class CreateConstraintTriggerParser {
    ...
    public static void parse(final PgDatabase database,
        ...
        parser.expect("CREATE","CONSTRAINT", "TRIGGER");

        final String triggerName = parser.parseIdentifier();
        final String objectName = ParserUtils.getObjectName(triggerName);

        final PgConstraintTrigger trigger = new PgConstraintTrigger();
        trigger.setName(objectName);
        
        if (parser.expectOptional("AFTER"))
        	trigger.setEventTimeQualification(PgConstraintTrigger.EventTimeQualification.after);
        ...
        if (!ignoreSlonyTrigger) {
            ...
            schema.getRelation(trigger.getRelationName()).addConstraintTrigger(trigger);
        }
    }
     ...
     */
    private CreateConstraintTriggerParser() {
    }
}
